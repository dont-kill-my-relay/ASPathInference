...
class CPrefix:
    ...
    def parseString(self, prefstr): #"xxx.xxx.xxx.xxx/xx"
        if not re.compile(r"^\d+\.\d+\.\d+\.\d+").match(prefstr):
            self.val = 0;
            self.prefixlen = 0;
            return;
        tmp = prefstr.split(".");
...
def isBogon(a):
    global bogonprefix;
    if a in bogonprefix or (a>=224 and a<256):
        return True;
    else:
        return False;
...
def readpathstr(line, allow_dup=False):
    ...
    for i in range(len(line) + 1):
        if i < len(line) and line[i] in "0123456789.":
            tmpstr = tmpstr + line[i];
        else:
            if len(tmpstr) > 0:
                if tmpstr.find('.') > 0:
                    tmpasns = tmpstr.split('.')
                    tmpasn1 = int(tmpasns[0])
                    tmpasn2 = int(tmpasns[1])
                    if tmpasn1 > 65535 or tmpasn2 > 65535:
                        raise Exception('Illegal AS number in %s' % line)
                    tmpasn = (tmpasn1 << 16) + tmpasn2
                    print tmpstr, tmpasn
                else:
                    tmpasn = int(tmpstr)
                tmpstr = ''
                if is_valid_asn(tmpasn) and \
                        (allow_dup or len(path) == 0 or path[-1] != tmpasn):
                    path.append(tmpasn);
    return path;
...
def gmmktime(tmstr, fmt):
    return time.mktime(time.strptime(tmstr, fmt)) - \
            time.mktime(time.strptime("1970-01-01 00:00:00", "%Y-%m-%d %H:%M:%S"))


def gmmktime2(tmstr):
    return gmmktime(tmstr, "%Y-%m-%d %H:%M:%S")
